...
This is why SIMD code is [branchless][branchless-code] by default.
...
This mask can then be composed with [bitwise operations][simd-bitwise] to filter specific lanes.
...
~~~~exercism/caution
NaN compares as unordered: it is neither less than, equal to, nor greater than anything, including itself.
The ordered predicates, such as `cmpltps`, yield false when either operand is NaN, while the unordered predicates yield true.
This is why `cmpordps` and `cmpunordps` exist, to test for NaN explicitly.
~~~~
...
AVX gives the compares, blends and `ptest` a `v`-prefixed form on wider registers.

The variable blend changes shape.
The mask is no longer the implicit `xmm0` but an explicit fourth operand:
...
AVX-512 extensions change the model more deeply.

First, it adds a new class of 64-byte SIMD registers, called `zmm`.
They expand corresponding `ymm` registers, in the same way `ymm` registers expand `xmm` registers.

Second, those extensions also create a whole new class of **opmask registers**, `k0` through `k7`, whose primary purpose is to serve as masks for SIMD instructions.
Comparisons no longer write to vectors, but to one of these new registers.
They can also be used directly on many instructions as predicates with a `{k}` write-mask, selecting lanes _directly_:
...
Finally, there are new instructions to operate on those new registers, including copying them into general-purpose registers.
This makes `movmsk` instructions unnecessary, and there is no support for `movmsk` instructions on `zmm` registers.
