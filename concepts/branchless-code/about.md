...
The [conditionals][conditionals-concept] concept introduces `jcc`, the family of instructions for branching based on a condition.
...
It uses a [branch predictor][branch-predictor] to guess the outcome of a condition and starts _speculatively_ executing the predicted path.
...
The family of instructions [cmovcc][cmov] copies the source operand into the destination only if a specific condition is met.
...
1. `l` and `g` are used for signed comparisons; `b` and `a` are used for unsigned.
...
The family of instructions [setcc][set] sets the destination operand to `1` if a specific condition is met, and to `0` otherwise.
...
~~~~exercism/advanced
A thorough reference on branch prediction for x86-64 CPUs can be found in the third chapter [of this reference book][book-chapter].
