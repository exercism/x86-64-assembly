...
; | `rcx`    | temporary    | integer | number of adjacent mines                      |
...
annotate:
        ...
        cld
        mov     rbx, rsi
        mov     al, [rbx]               ; read first byte of minefield
        test    al, al
        jz      .return
...
.update_next_column:
        ...
        mov     al, [r9 + r13]          ; minefield square
        cmp     al, '*'
        je     .write_square            ; jump ahead if we have reached a mine

        xor     rcx, rcx                ; number of adjacent mines
        mov     r11, r8
        sub     r11, rdx
...
.next_adjacent:
        ...
        jz      .write_square
        mov     al, '0'
        add     al, cl                  ; mine count, as ASCII digit
